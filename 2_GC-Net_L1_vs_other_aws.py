...
var_list1 = [ 'ISWR','OSWR', 'Alb',
             # 'TA1','TA2', 'P',
             # 'RH1',  #'RH2','SH1','SH2','VW1','VW2','DW1','DW2', 'LHF', 'SHF'
             ]
var_list2 =  [  'dsr_cor',  'usr_cor','albedo',
              # 't_l', 't_u', 'p_u']+[
    # 'rh_u',
    ] #+ ['rh_l','qh_l', 'qh_u' 'wspd_l', 'wspd_u','wdir_l','wdir_u']+ [ 'dlhf_u', 'dshf_u']
ylabels = [
...
for var1, var2, lab in zip(var_list1, var_list2, ylabels):
    ...
    for i in range(comp_matrix.shape[0]):
        ...
        if os.path.exists('Data/GEUS stations/'+name_sec+'_hour_v01.csv'):
            ...
            df_sec = df_sec.set_index('time')
            df_sec.index = df_sec.index + pd.Timedelta('1H')
        elif os.path.exists('Data/data_DYE2_Samira_hour.txt'):
            df_sec = gnl.load_ucalg('Data/data_DYE2_Samira_hour.txt')
            df_sec.index = df_sec.index + pd.Timedelta('1H')
        ...
        ax[count].plot(df_sec[var2], label = name_sec, # 'secondary AWS',
                       alpha=0.8)
        ...
        # if count==0:
        ax[count].legend(loc='upper right', bbox_to_anchor=(1, 1))
        ax[count].set_title(ABC[count]+') '+var2, #site+ ' AWS vs. '+name_sec+' AWS',
             loc='left')
...
df_gcn =df_gc.loc[slice("2016-05-29", "2016-05-29")]
df_ucalg =df_sec.loc[slice("2016-05-29", "2016-05-29")]
...
# Plot 1
axs[0].plot(df_gcn.SZA, df_gcn.ISWR, marker='o', ls='None', label='GC-Net')
axs[0].plot(df_gcn.SZA, df_ucalg.dsr_cor, marker='^', ls='None', label='U. Calg')
axs[0].set_ylabel("DSR")
axs[0].legend()

# Plot 2
axs[1].plot(df_gcn.SZA, df_gcn.OSWR, marker='o', ls='None', label='GC-Net')
axs[1].plot(df_gcn.SZA, df_ucalg.usr_cor, marker='^', ls='None', label='U. Calg')
axs[1].set_ylabel("USR")
axs[1].legend()

# Plot 3
axs[2].plot(df_gcn.SZA, df_gcn.Alb, marker='o', ls='None', label='GC-Net')
axs[2].plot(df_gcn.SZA, df_ucalg.albedo, marker='^', ls='None', label='U. Calg')
axs[2].set_ylabel("Albedo")
axs[2].set_xlabel("Solar Zenith Angle (degrees)")
axs[2].legend()
