...
#    df.columns =['site', 'year', 'month', 'day', 'hour', 'wdir', 
#                     'wspd', 'wsf', 'pres', 'ta_2', 'ta_10', 'ta_top', 'rh', 'rf']
...
# df_sum['time'] = [pd.Timestamp(df_sum.year[i].values,
#                                df_sum.month[i].values,
#                                df_sum.day[i].values,
#                                df_sum.hour[i].values)
#                   for i in df_sum.year.values)-1)]
# for i, y in enumerate(df_sum.year.values):
#     tmp = datetime.datetime(df_sum['year'].values[i],
#                             df_sum['month'].values[i],
#                             df_sum['day'].values[i],
#                             df_sum['hour'].values[i])
#     df_sum.iloc[i,-1] = tmp.replace(tzinfo=pytz.UTC)
